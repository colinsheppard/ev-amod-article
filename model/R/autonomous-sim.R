...
animate.soln <- function(sys){
  for(node in u(sys$node)){
    ...
    #system(pp('ffmpeg -framerate 30 -i ',ev.amod.shared,'model/results/soln-node',node,'-img%05d.png -s:v 800x1000 -c:v libx264 -profile:v high -crf 20 -pix_fmt yuv420p -r 30 ',ev.amod.shared,'model/results/soln-node',node,'.mp4'),intern=T,input='Y')
  }
}
